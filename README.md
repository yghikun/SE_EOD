...
- `outputs/ext4/suspicious_candidates.csv`
- 共 `38` 条静态可疑候选。
- 覆盖 `10` 个 ext4 C 文件。
- 类型分布：
  - `missing_cleanup`: `8`
...
- `outputs/ext4/llm_review_tasks.jsonl`
...
- `outputs/ext4/deepseek_reviews.jsonl`: `140` 条 DeepSeek 复核记录。
- `outputs/ext4/deepseek_true_candidates.jsonl`: `25` 条模型判为 `true_candidate` 的记录。
- `outputs/ext4/manual_bug_candidates_to_verify.md`: 对部分 DeepSeek true candidates 做了人工整理和后续验证计划。
...
python -m src.main \
  --linux linux-v6.8-fs \
  --out outputs/ext4/error_paths.csv \
  --check-candidates \
  --candidates-out outputs/ext4/suspicious_candidates.csv \
  --rank-evidence \
  --wrapper-summaries configs/wrapper_summaries.json \
  --enable-ownership-transfer-hints \
  --ranked-candidates-out outputs/ext4/ranked_candidates.jsonl \
  --candidates-with-evidence-out outputs/ext4/candidates_with_evidence.csv
```
...
python -m src.main \
  ...
  --candidates-out outputs/ext4/suspicious_candidates.csv \
  --rank-evidence \
  --ranked-candidates-out outputs/ext4/ranked_candidates_v1_no_exceptions.jsonl \
  --candidates-with-evidence-out outputs/ext4/candidates_with_evidence_v1_no_exceptions.csv
```
...
python -m src.main \
  ...
  --enable-ownership-transfer-hints \
  --wrapper-summaries configs/wrapper_summaries.json \
  --ranked-candidates-out outputs/ext4/ranked_candidates_v1_1.jsonl \
  --candidates-with-evidence-out outputs/ext4/candidates_with_evidence_v1_1.csv
```
...
python scripts/validate_v1_1.py \
  --v1-ranked outputs/ext4/ranked_candidates_v1_no_exceptions.jsonl \
  --v1-1-ranked outputs/ext4/ranked_candidates_v1_1.jsonl \
  --report-out outputs/ext4/v1_1_validation_report.md \
  --limit 20
```
...
```text
outputs/ext4/manual_review_labels.jsonl
```
...
python -m src.main \
  ...
  --enable-ownership-transfer-hints \
  --wrapper-summaries configs/wrapper_summaries.json \
  --manual-review-labels outputs/ext4/manual_review_labels.jsonl \
  --ranked-candidates-out outputs/ext4/ranked_candidates.jsonl \
  --candidates-with-evidence-out outputs/ext4/candidates_with_evidence.csv
```
...
`outputs/ext4/manual_review_labels.jsonl`。这些标签的 `reviewer` 和 `review_source` 都是
...
python -m src.main \
  ...
  --enable-ownership-transfer-hints \
  --wrapper-summaries configs/wrapper_summaries.json \
  --ranked-candidates-out outputs/ext4/ranked_candidates_v1_2_fp_model_no_manual.jsonl \
  --candidates-with-evidence-out outputs/ext4/candidates_with_evidence_v1_2_fp_model_no_manual.csv
```
...
python scripts/prepare_manual_review_queue.py \
  --ranked outputs/ext4/ranked_candidates_v1_2_fp_model_no_manual.jsonl \
  --llm-tasks outputs/ext4/llm_review_tasks.jsonl \
  --queue-jsonl-out outputs/ext4/manual_review_queue.jsonl \
  --queue-md-out outputs/ext4/manual_review_queue.md \
  --label-template-out outputs/ext4/manual_review_labels_todo.jsonl \
  --limit 20
```
...
- `outputs/ext4/manual_review_queue.jsonl`：结构化审查队列，便于脚本处理。
- `outputs/ext4/manual_review_queue.md`：带源码上下文的浏览版。
- `outputs/ext4/manual_review_labels_todo.jsonl`：待填写标签模板。
...
`outputs/ext4/manual_review_labels.jsonl`，再重新运行 ranking。ranker 会忽略说明行和未填写的
TODO verdict。
...
python scripts/validate_v1_2_review_feedback.py \
  --baseline-ranked outputs/ext4/ranked_candidates_v1_2_fp_model_no_manual.jsonl \
  --feedback-ranked outputs/ext4/ranked_candidates.jsonl \
  --review-labels outputs/ext4/manual_review_labels.jsonl \
  --report-out outputs/ext4/v1_2_review_feedback_report.md \
  --limit 20
```
...
`outputs/ext4/v1_2_review_feedback_report.md`。
...
```bash
python -m src.main --linux /path/to/linux --out outputs/ext4/error_paths.csv
```
...
python -m src.main \
  ...
  --out outputs/btrfs/error_paths.csv \
  --check-candidates \
  --candidates-out outputs/btrfs/suspicious_candidates.csv \
  --rank-evidence \
  --protocols-dir configs/btrfs_resource_protocols \
  --enable-ownership-transfer-hints \
  --wrapper-summaries configs/btrfs_wrapper_summaries.json \
  --ranked-candidates-out outputs/btrfs/ranked_candidates.jsonl \
  --candidates-with-evidence-out outputs/btrfs/candidates_with_evidence.csv \
  --build-llm-tasks \
  --llm-tasks-out outputs/btrfs/llm_review_tasks.jsonl
```
...
python -m src.main \
  --rank-evidence \
  --candidates-in outputs/ext4/suspicious_candidates.csv \
  --ranked-candidates-out outputs/ext4/ranked_candidates.jsonl \
  --candidates-with-evidence-out outputs/ext4/candidates_with_evidence.csv
```
...
python -m src.main \
  ...
  --wrapper-summaries configs/wrapper_summaries.json \
  --ranked-candidates-out outputs/ext4/ranked_candidates.jsonl \
  --llm-tasks-out outputs/ext4/llm_review_tasks.jsonl
```
...
python -m src.main \
  ...
  --llm-tasks-out outputs/ext4/llm_review_tasks.jsonl \
  --run-deepseek-review \
  --deepseek-reviews-out outputs/ext4/deepseek_reviews.jsonl \
  --deepseek-true-candidates-out outputs/ext4/deepseek_true_candidates.jsonl \
  --deepseek-model deepseek-v4-pro \
  --deepseek-reasoning-effort max \
  --deepseek-limit 5
```
...
### `outputs/ext4/ranked_candidates.jsonl`
...
### `outputs/ext4/candidates_with_evidence.csv`
...
### `outputs/ext4/manual_review_queue.jsonl`
...
### `outputs/ext4/manual_review_queue.md`

同一队列的浏览版，适合直接阅读源码上下文并填写审查结论。

### `outputs/ext4/manual_review_labels_todo.jsonl`

review feedback TODO 模板，不应直接作为 `--manual-review-labels` 输入。填完后，把有效 JSON object
复制到 `outputs/ext4/manual_review_labels.jsonl`。未填写的占位 verdict 会被 loader 忽略。

### `outputs/ext4/ranked_candidates_v1_no_exceptions.jsonl`

用于 v1.1 验收的 v1 baseline ranked 输出。它保留协议 ranking 和 wrapper summary 默认行为，
但不启用 `--enable-ownership-transfer-hints`。

### `outputs/ext4/ranked_candidates_v1_1.jsonl`

用于 v1.1 验收的 exception-aware ranked 输出，启用了 wrapper summaries 和 ownership transfer hints。

### `outputs/ext4/ranked_candidates_v1_2_no_manual.jsonl`
...
### `outputs/ext4/ranked_candidates_v1_2_fp_model_no_manual.jsonl`
...
### `outputs/ext4/v1_1_validation_report.md`
...
### `outputs/ext4/deepseek_reviews.jsonl`
...
### `outputs/ext4/deepseek_true_candidates.jsonl`

从 DeepSeek 结果中提取出的 `verdict == "true_candidate"` 记录，方便后续人工验证。

### `outputs/ext4/manual_bug_candidates_to_verify.md`
