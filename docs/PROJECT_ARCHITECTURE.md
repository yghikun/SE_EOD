...
或捕获的调用返回值绑定对象；A-C 仍是扁平兼容配置。Protocol E 尚未支持自动 cleanup
宏、allocation publication 或跨调用边界 ownership transfer。
...
这些文件仍不是 validation 结果；所有 10 条 rule maturity 仍保持 development。
