...
The repository keeps only the components needed for failure-path filesystem
metadata residual analysis.
...
src/function_extractor.py
  Function slicing and extraction helpers for source files.

src/metadata_scope.py
  Versioned filesystem metadata scope contract and confirmed-bug scope labels.

src/metadata_residual.py
  Filesystem metadata residual-analysis data model.
```
...
outputs/confirmed_bugs.md
  Evidence ledger for motivating and evaluating filesystem metadata residual analysis.
...
uses failure-path filesystem metadata residual analysis instead.
