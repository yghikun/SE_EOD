...
> 本文档以当前仓库代码为唯一事实来源，描述截至 2026-07-17 的实现。文档中的“已实现”表示有代码和回归测试支撑；“部分实现”和“未实现”不应在论文或实验报告中被表述为完整能力。

SE-EOD 是一个面向 Linux 文件系统 C 代码的错误路径与资源义务分析原型。它的输出是待复核候选，不是 confirmed bug。它将三件事明确分层：

1. 从源码中提取错误路径并跟踪资源状态。
2. 将静态路径转换为 `missing_cleanup`、`partial_cleanup`等候选。
3. 使用协议、历史修复、人工标签和 LLM 信号排序，但不让排序信号反向篡改静态语义。
...
Linux source tree
  ...
  -> parse_c_file(): tree-sitter 解析和质量标记
  -> extract_functions(): 函数级 IR
...
| 层级 | 模块 | 当前责任 |
|---|---|---|
| 调度 | `src/main.py` | CLI、阶段组合、输出、统计和 warning |
| 文件发现 | `src/file_walker.py` | 递归扫描指定文件系统子目录下的 `.c` |
| 前端 | `src/parser.py` | tree-sitter C 解析，记录 parser quality 和 warning |
| 函数 IR | `src/function_extractor.py` | 函数、签名、参数、源码位置、AST 节点和 `analysis_quality` |
...
| 候选规则 | `src/candidate_rules.py` | missing、partial、swallowed、stale retry 候选 |
| 候选 I/O | `src/candidate_checker.py` | 读错误路径、写候选并统计 |
...
- `--build-llm-tasks`：从候选或 ranking 结果构建任务；
- `--run-deepseek-review`：要求 `DEEPSEEK_API_KEY`；
- `--extract-deepseek-true-candidates`：不调用模型，仅从既有 review JSONL 提取子集。

### 4.3 默认置信度边界

`src.main` 默认仅保留 `high` 和 `medium` 错误路径。`--include-low-confidence` 才会输出 `low/uncertain`。这个过滤发生在候选规则之前，因此文本降级分析不会默认伪装成正常候选。
...
```text
file, name, signature, source, body
start_line, end_line, body_start_line
ast_node, body_node, parse_tree
source_start_byte, body_start_byte, file_bytes
parameters
analysis_quality
```

它是带位置和 AST 引用的函数级 IR，但不是 typed IR，也没有 SSA value、显式 memory object 或 points-to set。
...
```text
fallthrough, true, false, goto, backedge, return, break, continue, unknown
```

当前 `switch_statement` 和 `case_statement` 会记入 `unsupported_nodes`，而不是获得完整 switch CFG 语义。无法解析的 `goto` 会连到 exit 并记录 `unresolved_goto:<label>`。
...
这些统计在主流程 stdout 中输出。主流程还从已生成的静态候选统计 `candidates_with_unknown_guard`、`candidates_with_loop_multiplicity`、`candidates_with_incomplete_cfg` 和 `candidates_with_widening`，用于区分分析器不确定性的来源。
...
AST 路径现在记录 `condition_start_byte/condition_end_byte`，并绑定 CFG 的 `edge_id/source_block/target_block/edge_kind`。同一函数内两个文本相同的 `if (ret)` 因 byte range 和 edge ID 不同而可区分。历史字段名仍为 `cfg_witness`，但 JSON 内明确记录 `kind=cfg_analysis_snapshot`，并包含 source state 数、可达 blocks/return blocks、scope-unwind edges、`cfg_complete/unsupported_nodes`、widening/truncated，以及每个可达 return 的析取状态快照。快照包含资源 ID/state/multiplicity/uncertainty、path facts 和尚未求值的 pending summary effects（包括 call site、result version 和 cardinality）。

这是出口分析快照，不是逐语句可重放 witness：它尚不记录 state predecessor、incoming edge、每个中间 block 的 before/after 状态和 resource transition。
...
同一 acquire site 经 backedge 再次执行、且上一实例仍为 `ACQUIRED/MAY_ACQUIRED` 时，实例提升为 `multiplicity=many`，记录 `loop_multiple_instances`。默认 `effect_cardinality=one`的 release/transfer/escape 只能将其保持为 `MAY_ACQUIRED`；只有 reviewed `effect_cardinality=all` 或 acquire 合约的 `release_cardinality=all` 能解除 `many` 义务。`unknown` 与 `one` 一样保守保留。
...
`ResourceFlowState.pending_summary_effects` 保存尚待调用返回值分支证明的出口敏感 effect，其身份包含 `call_site_id/resource_id/result_var/result_version/action/strength/exit_class/return_guard/effect_cardinality`。赋值、自增减或通过 output parameter 修改结果变量时会推进 definition version 并失效旧 pending effect；scope exit 也会丢弃绑定到离开作用域的 result symbol。分支 edge 添加新 facts 后才判断 `return_guard`。widening 只保留两侧完全相同的 pending effect，不会合并不同调用或版本。
...
嵌套 compound CFG 带 `scope_enter/scope_exit`。`ResourceFlowState.scope_frames` 记录进入作用域前的 alias、scope cleanup、函数指针目标、definition version 和相关 path facts；仅对该作用域内声明的名字恢复绑定。`goto/break/continue/return` 边携带 `scope_unwind`，edge transfer 会按跨越的作用域层数调用 `_leave_scope()`，因此非正常离开也会执行 scope cleanup 并恢复外层绑定。资源实例本身不会随名字离开作用域而删除。

这仍不是完整 SSA/context-sensitive symbol 模型：循环中同一 acquire site 使用一个带 `many` 标记的抽象实例；一般 `path_facts`、error provenance 和 function-pointer symbol 仍以规范化文本为主，scope frame 只对已识别声明做同名恢复；复杂聚合、数组下标相等性和跨函数 points-to 也未覆盖。`function_target_complete` 是局部赋值闭包状态，不是全程序 points-to 完备性证明。
...
自动 release/out-parameter effect 只有在 `analysis_quality=tree-sitter`、`unsupported_nodes` 为空、callee 和实参映射可精确解析，且删除相关 effect block 后无法再到达 CFG exit 时，才推导为 `must`。`switch/case`、`unresolved_goto`、degraded parser、绕过调用的出口或不精确调用均强制降为 `may`。无 AST 输入不能生成自动 `must`。确定 effect 的 `must_reason` 输出 `complete_cfg/cfg_postdominating_effect/exact_callee/exact_argument_mapping`等独立证据；reviewed seed 则记录 `reviewed_seed`。
...
它构建调用图并做全局不动点迭代，能处理有限的递归/互递归传播，但当前并没有显式按 SCC 调度或 context-sensitive call string。
...
历史 `resource_ownership_transfers` 同样只是静态阶段的兼容 hint：它可降低确定性并记录 provenance，不得删除 held resource。

### 10.3 reviewed suppression
...
AST 路径 fingerprint 依赖 `file/function/condition_start_byte/condition_end_byte/branch_taken/cfg_edge_kind/candidate_type/error_line`；没有 byte range 的降级路径才回退到 `path_id`。obligation ID 在此基础上再绑定 `resource_id + missing action`，因而同一路径上的 `kfree(ptr)` 和 `brelse(bh)` 可分别审查。ranking 仍以路径行聚合展示，并输出 `obligation_candidate_ids` 列表。为读取已有人工/LLM 标签，同时输出并索引原 SHA-1/12 的 `legacy_candidate_id/legacy_llm_task_id`；新产物不应继续生成 legacy ID。
...
Historical fix 要求精确匹配文件、函数、候选类型和影响行，它只附加到已有候选，不反向生成或删除候选。

Manual review 查找顺序支持 obligation ID、当前 path/task ID、legacy ID 和 `path_id`，并带来 source-aware score adjustment。当前排序记录仍按路径聚合，因此同一路径上多个义务的冲突标签尚未拆成独立 ranking 行；UI/标注流程应优先保留 obligation ID。
...
`held_resources` 中包含 `resource_id/generation/multiplicity/release_cardinality/validity_guard/validity_guard_source`、acquire 位置、预期 release、scope cleanup、`ownership_state` 和 `uncertainty_causes`。`cfg_witness` 是历史字段名，对象的 `kind=cfg_analysis_snapshot`，包含入口 edge identity、byte range、scope unwind、CFG completeness、source state 数、可达 block、return 出口状态快照及 widening/truncated 标志。`released_cleanup_candidates` 和 `partial_cleanup` 是逐个 return block 的逐个 `ResourceFlowState` 计算；不会将不同析取状态的 released/missing 集合后伪造 partial cleanup。
...
```text
candidate_id, path_candidate_id, obligation_candidate_ids
...
```text
candidate_id, path_candidate_id, obligation_candidate_ids, legacy_candidate_id
...
| 能力 | 证据 |
|---|---|
| tree-sitter 函数级 AST | `parser.py`, `function_extractor.py` |
| 解析质量降级隔离 | `analysis_quality`, low-confidence filter |
...
| action cardinality | `release_cardinality/effect_cardinality`, reviewed `all` effect |
| label 后 CFG 终点义务检查 | `cleanup_outcome_cfg` / `missing_cleanup_candidates_cfg` |
| if true/false branch binding | `ErrorPath.branch_taken` |
| CFG analysis snapshot | byte range、edge ID、scope unwind、CFG completeness、return states、widening/truncated |
...
| pending effect identity | `call_site_id`, result definition version, scope/reassignment invalidation |
...
| alias/scope | 简单变量、swap、compound 遮蔽、非正常 scope unwind、部分字段/数组表达式 | 一般 symbol identity、SSA、聚合对象和 points-to 不完整 |
| 函数指针 | 局部目标集及 completeness；字段/表调用保守识别 | completeness 不是全程序证明；间接调用图、宏表和跨编译单元不完整 |
| 路径可行性 | 简单相等/空值/正负谓词 | 无一般布尔化简和 SMT |
| 跨函数 | 参数/返回 effect 全局不动点；must/may；reviewed success/error guard；规范状态 wrapper 传播 | 不自动推导一般出口类别，无显式 SCC 调度、context sensitivity 和完整 alias summary |
| witness | analysis snapshot：acquire/guard、edge identity、scope unwind、CFG completeness、return states | 无 state predecessor 和可反向重放的 transition trace |
| 证据排序 | 多源启发式分数 | 未校准，相关证据可能重复计分 |
...
| 复杂 alias | 别名 release 未匹配 | 文本 symbol/聚合 alias 混淆 | resource ID + alias/swap + scope frame/unwind；仍需完整 symbol/points-to |
...
| 循环多实例 | 抽象 `many` 可能保留已全部释放的候选 | 错误 `all` 合约可过度解除 | 按 modified variables 失效 facts；默认 one，仅 reviewed all 清空 many |
| 条件 cleanup | 若谓词不可判定则保守报告 | 静态列表曾会误抑制 | CFG 可达 return 状态是语义来源 |
| 文本降级 | 语法误识别 | 函数/路径遗漏 | 强制 low，默认不输出，不进 summary |
| ranking 相关证据 | 兼容总分仍可能过高 | 例外 hint 可过度降分 | protocol/history 分维度展示，不用总分做 bug 概率 |
...
18. `resource_ownership_transfers` 兼容 hint 不得过滤 held resource。
19. pending effect 必须绑定 call site 和 result definition version，重赋值后不得由旧 guard 触发。
20. `partial_cleanup` 必须在同一 return block 的同一析取 state 内判定。
21. `multiplicity=many` 只能被 reviewed `cardinality=all` 确定解除。
...
这套架构的实用价值是可以在多个 Linux 版本和文件系统上稳定生成可复现的错误路径与候选语料。它的主要研究风险仍是前端缺少编译上下文、循环只做粗粒度 multiplicity/cardinality 抽象、一般 symbol identity 和聚合别名不完整、一般 success/error effect 尚不能从 callee CFG 自动推导、未迁移配置仍使用 compatibility acquire guard、analysis snapshot 还不是 predecessor trace，以及 ranking 未校准。
